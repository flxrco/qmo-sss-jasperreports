...
public class RenderManager {
    ...
    /**
     * Generates the report that on the start of the queue. The dataset to be
     * used is from the Connection argument
     *
     ...
     * @throws EmptyRenderQueueException if this method was called even though
    ...
    public Map<String, Object> render(Connection con) throws JRException, EmptyRenderQueueException {
        ...
        synchronized (args) {
            tempArgs = this.args.poll(); //dequeues and then passes the values to the tempargs
            dest = destPath.poll(); //dequeues the path from the destPath queue
        }
    ...
    /**
     * Generates the report that on the start of the queue. It will use the
     * Connection from the CompileManager to fetch the dataset
     *
     * @return the Map used as the parameters for the JasperFillManager
     * @throws JRException JRException if a Jaspersoft-related error was
     * encountered
     * @throws EmptyRenderQueueException if this method was called even though
     * the queue is already empty
     */
    ...
    /**
     * Generates all of the reports that are in the queue. It uses the
     * Connection from the CompileManager to fetch the dataset.
     *
     * @throws JRException JRException if a Jaspersoft-related error was
     * encountered
